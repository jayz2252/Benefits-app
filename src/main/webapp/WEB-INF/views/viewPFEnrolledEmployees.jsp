				...
				<div class="col-md-12 white">
					...
					<div class="row">
						<div class="col-sm-6 col-md-9 py-1 px-1">
							<h4 class="h4-responsive">PF Enrolled Employees</h4>
						</div>
					...
					<div class="col-md-12">
						<table
							...
							<tbody>
								<c:forEach items="${pfEmployees}" var="pfEmployee">
									<tr>
										<td><c:if test="${pfEmployee.status eq 'SUBM'}">
												<div class="remember-label">
													<div>
														<input class="chk" type="checkbox"
															id="selectPF${pfEmployee.pfEmployeeId}" name="selectPF"
															value="${pfEmployee.pfEmployeeId}" /> <label
															for="selectPF${pfEmployee.pfEmployeeId}"></label>
													</div>
												</div>
											</c:if></td>
										<td><a
											href="<%=request.getContextPath()%>/home/controlPanel/pf/search/viewDetails/${pfEmployee.pfEmployeeId}">${pfEmployee.employee.firstName}
										...
										<td><c:choose>
												<c:when test="${pfEmployee.status eq 'SUBM'}">Submitted</c:when>
												<c:when test="${pfEmployee.status eq 'HR_APPR'}">Approved</c:when>
												<c:when test="${pfEmployee.status eq 'SL_SUBM'}">SL Submitted</c:when>
												<c:otherwise>Rejected</c:otherwise>
											</c:choose></td>
										<td><c:if test="${pfEmployee.status eq 'SUBM'}">
												<a
													href="<%=request.getContextPath()%>/home/controlPanel/pf/search/approvalView/${pfEmployee.pfEmployeeId}">
													Approve</a>
											</c:if></td>
										<td><a
						...
						<form id="approveHiddenId"
							action="/home/controlPanel/pf/search/approveSelected">
