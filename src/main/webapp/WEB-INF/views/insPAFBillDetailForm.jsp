...
<body>
	...
	<div id="main">
		<div class="wrapper">
			...
			<form
				action="<%=request.getContextPath()%>/home/controlPanel/insurancePlans/optedEmployees/PAFbillDetails/new/save/${billNo}"
				method="post">
...
<input type="hidden" value="${categories}" name="category" id="category">
					<div class="row">
						...
						<div class="col-md-12">
							...
							<table id="billDetails" class="responstable dynamicTable">
								...
								<tbody>
									...
									<c:forEach items="${bean.billDetails}" var="detail" varStatus="i">
										<tr>
									...
									<tr>
										...
										<td><select id="categoryId${countLoop}" name="categoryId${countLoop}">
												<option disabled selected>Choose Category</option>
												<c:forEach items="${categories}" var="category">
													<option value="${category.categoryId}" id2="${category.unitCost}" onclick="setUnitCost(this)">${category.categoryDesc}</option>
												</c:forEach>
										</select></td>
										<td><input type="text" name="item${countLoop}" id="item${countLoop}" /></td>
										<td><input type="text" name="unitCost${countLoop}" id="unitCost${countLoop}" /></td>
										<td><input type="text" name="quantity${countLoop}" id="quantity${countLoop}" onchange="changeTotal(this)"/></td>

										<td><input type="text" name="totalAmt${countLoop}" id="totalAmt${countLoop}"
											value="" /></td>
						...
						<div class="col-md-12">
							...
							<div class="form-group">
								...
								<input type="text" id="totalAmount" name="totalAmount"
									 value="${total}" /><label for="totalAmount">Bill Total(Rs.)</label>
							</div>
						</div>
					</div>
...
row = $(e).closest('tr'); 
	...
	$('#totalAmount').val(parseFloat($('#totalAmt'+rowIndex).val())+parseFloat($('#totalAmount').val()));
	}
